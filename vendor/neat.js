...
export function NEAT(config) {
	...
	this.bestCreature = function () { // Returns the index of the best creature from the previous generation.
		...
		const bestCreature = this.oldCreatures[index];
		return [index, bestCreature.fitness, bestCreature];
	}
...
function Creature(model) {
	...
	this.desicion = function () { // Some spaghetti code that returns the desicion of the creature.
		...
		for (let i = 0; i < this.network.layers[this.network.layers.length - 1].nodes.length; i++) {
			values.push(this.network.layers[this.network.layers.length - 1].nodes[i].value);
		}
		return values;
	}
}
...
export let activation = { // Supported activation functions.
	RELU: function (x) {
		if (x > 0) return x;
		else return 0;
	},
...
export let crossover = { // Crossover methods.
...
export let mutate = { // Mutation function (More to come!).
